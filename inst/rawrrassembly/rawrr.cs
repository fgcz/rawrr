    ...
    */
            ...
            public static void PrintHeaderAsRcode(this IRawDataPlus rawFile, string filename)
	    {
            ...
            public static void GetIndex(this IRawDataPlus rawFile){
                ...
                int idxCharge = rawFile.GetIndexOfPattern(trailerFields, "Charge State");
                int idxMasterScan = rawFile.GetIndexOfPattern(trailerFields, "Master Scan Number:");
                int idxDependencyType = rawFile.GetIndexOfPattern(trailerFields, "Dependency Type:");
                int idxMonoisotopicmZ = rawFile.GetIndexOfPattern(trailerFields, "Monoisotopic M/Z:");

                double charge = -1, precursorMass;
                double monoIsotopicMz = -1;
                int masterScan = -1, dependencyType = -1;
        ...
        private static int GetIndexOfPattern(this IRawDataPlus rawFile, HeaderItem[] trailerFields, string pattern = "Charge State", int? scanNumber = null){
            ...
            var trailerFields = rawFile.GetTrailerExtraHeaderInformation();
		    int idxCharge = rawFile.GetIndexOfPattern(trailerFields);
             	    int firstScanNumber = rawFile.RunHeaderEx.FirstSpectrum;
            	    int lastScanNumber = rawFile.RunHeaderEx.LastSpectrum;
		        int charge = -1;
		        double pc=-1;
                    ...
                    new System.IO.StreamWriter(filename))
                {
             	    for  (int scanNumber = firstScanNumber; scanNumber < lastScanNumber; scanNumber++){
                        ...
                        var scanEvent = rawFile.GetScanEventForScanNumber(scanNumber);
			var scanFilter = rawFile.GetFilterForScanNumber(scanNumber);
            idxCharge = rawFile.GetIndexOfPattern(scanTrailer);
            ...
            public static void WriteTrailerValues(this IRawDataPlus rawFile, string label)
	    {
		    ...
		    int idx = -1;
		    try{
		    	idx = rawFile.GetIndexOfPattern(trailerFields, label);
    ...
    namespace FGCZ_Raw
    {
        ...
        internal static class Program
        {
			...
			try {
                ...
                {
                    Console.WriteLine("Error accessing RAWFileReader library! - " + ex);
                }
