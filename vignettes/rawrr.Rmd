...
The R script that renders the html page is also available as supplementary information. The correspoding [R markdown file](https://github.com/fgcz/rawrr/blob/master/vignettes/JPR_supplement.Rmd) is part of the `r BiocStyle::Biocpkg('rawrr')` package and can be processed locally after downloading a snapshot of the above described input data from [MSV000086542](https://massive.ucsd.edu/ProteoSAFe/dataset.jsp?accession=MSV000086542) [@MSV000086542]. In summary, this shows how scalable analysis pipelines can be constructed starting from basic building blocks. It demonstrates that `r BiocStyle::Biocpkg('rawrr')`'s data access mechanism works for all types of Orbitrap instrument models.

## Benchmark

```{r benchmark, message = FALSE, fig.cap="`rawrr:::.benchmark using rawrr::readSpectrum` spectra per second.", fig.small=TRUE}
set.seed(1)
seq(1, 4) |>
...
```{r rawrrAssemblyPath, echo=TRUE}
rawrr:::.rawrrAssembly()
```

prints the `rawrr` assembly version

```{r rawrrAssemblyVersion, echo=TRUE}
system2(rawrr:::.rawrrAssembly(), args = c('version'), stdout = TRUE)
```
